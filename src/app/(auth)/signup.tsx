  ...
  return (
    <SafeAreaView edges={["bottom"]} style={styles.safeArea}>
      <View style={styles.container}>
        ...
        <Text style={styles.subtitle}>FootGo 계정을 만들고 Phase 2 온보딩을 시작하세요.</Text>
        <View style={styles.card}>
          ...
          <PrimaryButton label={isSigningIn ? "가입 중..." : "이메일로 회원가입"} onPress={() => { if (!validateForm()) { return; } void signUpWithEmail(normalizeEmail(email), password); }} isDisabled={isLoading || isSigningIn} />
          <PrimaryButton label="로그인" onPress={() => router.push("/(auth)/login")} variant="secondary" isDisabled={isLoading || isSigningIn} />
        </View>
